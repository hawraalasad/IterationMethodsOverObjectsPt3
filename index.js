...
function listAllCourses(students) {
  ...
  students.forEach((student) => {
    student.courses.forEach((course) => {
      if (
        allCourses.find((previousCourse) => previousCourse == course) ==
        undefined
      ) {
...
function removeCourseFromStudent(student, course) {
  ...
  newCourses = student.courses.filter((subject) => {
...
function getStudentsByCourse(course, students) {
  // Your code here
  onlyThisCourse = students.filter((student) => {
    if (student.courses.includes(course)) {
      return true;
    }
  });
  return onlyThisCourse;
}
